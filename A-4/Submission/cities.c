...
void readcsv(FILE *fcities, City* cities){
    ...
    while( fgets(buf, 500, fcities) ) /* Reading each row */
    {
        if (rows == 0) rows++; /* skipping header column */
        else
        {
            /* Reading each column */
...
int main(){
    ...
    char* newfile = "cities.csv";
    char* sortfile = "sorted.csv";

    FILE *fcities = fopen(newfile, "r");
    if (!fcities){
